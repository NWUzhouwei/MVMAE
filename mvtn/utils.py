...
def unit_spherical_grid(nb_points, return_radian=False, return_vertices=False):
    elev = []
    azim = []
    num_bands = int(np.sqrt(nb_points))  # 计算纬度带的数量
    points_per_band = nb_points // num_bands  # 每个纬度带的点数

    for i in range(num_bands):
        elevation = 90 + (i + 1) * 180 / (num_bands + 1)  # 将仰角限制在90到270度之间
        for j in range(points_per_band):
            azimuth = j * 360 / points_per_band
