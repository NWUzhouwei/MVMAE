...
from mvtn.mvtn_util import unit_spherical_grid, batch_tensor
...
class CircularViewSelector(nn.Module):
    def __init__(self, nb_views=12, canonical_elevation=35.0, canonical_distance=3.0, transform_distance=False, input_view_noise=0.0):
        super().__init__()
        ...
        # views_azim = torch.linspace(0.1, 0.1, self.nb_views+1)[:-1] 

        views_elev = torch.ones_like(
            views_azim, dtype=torch.float, requires_grad=False)*canonical_elevation
        # views_elev = torch.linspace(-180, 180, self.nb_views+1)[:-1] + 90
...
class MVTN(nn.Module):
    ...
    def __init__(self, nb_views=12, views_config="spherical", canonical_elevation=30.0, canonical_distance=1.0, transform_distance=False, input_view_noise=0.0, shape_extractor="pointnet", shape_features_size=512, screatch_feature_extractor=False):
        super().__init__()
